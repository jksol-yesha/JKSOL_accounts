...
export const getCategories = async ({ body, set, user, orgId, branchId }: ElysiaContext & { body: { branchId?: number | number[] | 'all', orgId?: number, financialYearId?: number } }) => {
    ...
    const items = await CategoryService.getAll(finalOrgId, finalBranchId, body.financialYearId, user);
    return {
        success: true,
        data: items
    };
...
export const exportCategories = async ({
    ...
    const groupedCategories = await CategoryService.getGroupedExportData(finalOrgId, finalBranchId, {
    ...
    if (format === 'pdf') {
        const html = CategoryService.buildPrintableHtml(groupedCategories);
        set.headers['content-type'] = 'text/html; charset=utf-8';
        set.headers['content-disposition'] = 'inline; filename="category-export.html"';
        return new Response(html, {
            headers: {
                'content-type': 'text/html; charset=utf-8',
                'content-disposition': 'inline; filename="category-export.html"'
            }
        });
    }

    const csvContent = CategoryService.buildExportCsv(groupedCategories);
    const fileName = `category-export-${new Date().toISOString().slice(0, 10)}.csv`;
...
export const createCategory = async ({ body, set, user, orgId }: ElysiaContext & { body: any }) => {
    if (!user || !orgId) throw new Error("Unauthorized: User or Organization not identified");
    try {
        const newCategory = await CategoryService.create({ ...body, orgId }, user.id);
        return {
            success: true,
            data: newCategory
        };
...
export const updateCategory = async ({ params: { id }, body, user, orgId, set }: ElysiaContext & { params: { id: string }, body: any }) => {
    ...
    try {
        ...
        if (e.code === 'ER_DUP_ENTRY' || e.message?.includes('Duplicate entry') || e.cause?.code === 'ER_DUP_ENTRY') {
            set.status = 409;
        ...
        set.status = 500;
        return { success: false, message: e.message || 'Failed to update category' };
    }
};
...
export const createSubCategory = async ({ body, set, user }: ElysiaContext & { body: any }) => {
    ...
    try {
        const newSub = await CategoryService.createSub(body, user.id);
...
export const updateSubCategory = async ({ params: { id }, body, user, set }: ElysiaContext & { params: { id: string }, body: any }) => {
    if (!user) throw new Error("Unauthorized");
    try {
        const updated = await CategoryService.updateSub(Number(id), body, user.id);
        return { success: true, data: updated };
    } catch (e: any) {
        if (e.code === 'ER_DUP_ENTRY' || e.message?.includes('Duplicate entry') || e.cause?.code === 'ER_DUP_ENTRY') {
            set.status = 409;
            return { success: false, message: 'Subcategory with this name already exists.' };
        }
        if (e.message?.includes('Subcategory not found') || e.message?.includes('Category not found')) {
            set.status = 404;
            return { success: false, message: e.message };
        }

        set.status = 500;
        return { success: false, message: e.message || 'Failed to update subcategory' };
    }
};
