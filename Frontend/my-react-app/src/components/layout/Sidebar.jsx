...
const Sidebar = ({ isCollapsed, isOpen, onClose, className }) => {
    ...
    const menuItems = [
        ...
        { icon: Settings, label: 'Settings', path: '/settings', permission: 'SETTINGS_MANAGE' },
    ];
    ...
    const filteredMenuItems = menuItems.filter(item => {
        ...
        // Optional: If we want to enforce other permissions (not requested but good practice if hasPermission implies it)
        // usage: if (item.permission && !hasPermission(item.permission)) return false; 
                ...
                <div className={cn(
                    ...
                    {filteredMenuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <item.icon
                                    size={item.label === 'Settings' ? 22 : 20}
                                    strokeWidth={item.label === 'Settings' ? 1.5 : 2}
                                    className={cn(
                ...
                {showHoverExpandPanel && hoveredItem && (
                    ...
                    >
                        <div className="flex h-full items-center gap-4 px-4 bg-black text-white shadow-lg shadow-black/25 rounded-xl border border-black">
                            <hoveredItem.icon
                                size={20}
                                strokeWidth={hoveredItem.label === 'Settings' ? 1.75 : 2}
                                className={cn(
                                    "shrink-0",
                                    "text-white"
                                )}
