...
const Transactions = () => {
    ...
    const parseTransactionTimestamp = (value) => {
        ...
        if (naiveDateTimeMatch && !hasExplicitTimeZone) {
            ...
            return fromZonedTime(localDate, selectedOrg?.timezone || 'Asia/Kolkata');
        }
    ...
    const formatTransactionClock = (value) => {
        if (!value) return null;
    ...
    const getTransactionTimeMeta = (txn) => {
        const createdAt = parseTransactionTimestamp(txn?.createdActionAt || txn?.createdAt);
        const updatedAt = parseTransactionTimestamp(txn?.latestUpdateActionAt || txn?.updatedAt);
        const hasCreatedAt = createdAt && !Number.isNaN(createdAt.getTime());
        const hasUpdatedAt = updatedAt && !Number.isNaN(updatedAt.getTime());
        const wasUpdated = hasCreatedAt && hasUpdatedAt && updatedAt.getTime() > createdAt.getTime() + 1000;
        const sourceDate = wasUpdated ? updatedAt : createdAt;
        const formattedTime = formatTransactionClock(sourceDate);
                    ...
                    <div className="relative lg:hidden flex-1 p-4 space-y-4 print:hidden overflow-y-auto min-h-0 no-scrollbar" aria-busy={loading}>
                        {showInitialLoader ? (
                            ...
                            paginatedTransactions.map((txn, index) => {
                                ...
                                const timeMeta = getTransactionTimeMeta(txn);
                                return (
                                        ...
                                        <tr key={txn.id} className="group hover:bg-gray-50/50">
                                            ...
                                            <td className="px-2 py-1.5 text-xs font-medium text-gray-700 whitespace-nowrap">
                                                <div className="flex flex-col leading-tight">
                                                    ...
                                                    {timeMeta && (
                                                        <span className="mt-0.5 text-[9px] font-semibold text-gray-400 normal-case">
                                                            {timeMeta.time}
                                                        </span>
                                                    )}
                                                </div>
