...
import {
    ...
    Download, X, FileSpreadsheet, ChevronDown, ArrowUpDown, Wifi, WifiOff, Paperclip, Eye, ExternalLink, User
} from 'lucide-react';
...
const AttachmentPreview = ({ attachmentPath, isOpen, onClose, className = "left-0 top-full mt-2" }) => {
    ...
    const fullUrl = getFullUrl(attachmentPath);
        ...
        <div className={cn(
            ...
            <div className="space-y-1.5 max-h-72 overflow-y-auto no-scrollbar">
                <a
                    ...
                    className="flex items-center gap-3 p-2 hover:bg-primary/5 rounded-lg transition-all group/item border border-transparent hover:border-primary/10"
                    onClick={(e) => e.stopPropagation()}
                >
...
const Transactions = () => {
    ...
    const { formatCurrency, formatDate, formatDateTime } = usePreferences();
    ...
    const { isConnected, on } = useWebSocket(socketBranchId);
    ...
    // Note: We don't depend on isConnected to avoid unregistering listeners during reconnections
    useEffect(() => {
    ...
    const groupedTransactions = useMemo(() => {
        ...
        raw.forEach(txn => {
            // Group by Date, Amount, Party, Type, and Notes to ensure they are the same transaction
            const date = txn.txnDate ? new Date(txn.txnDate).toISOString().split('T')[0] : '';
            const amount = Number(txn.amountBaseCurrency || txn.finalAmountLocal || txn.amountBase || 0).toFixed(2);
            ...
            const baseKey = `${date}|${amount}|${party}|${type}|${notes}`;
            const branchName = txn.branch?.name || (branches || []).find(b => String(b.id) === String(txn.branchId))?.name || 'Unknown';

            // Find an existing group with this baseKey that DOES NOT already have this branchName
            // If it already has this branchName, it implies this is a separate identical transaction in the same branch
            let foundGroup = groups.find(g => g.baseKey === baseKey && !g.branchNames.includes(branchName));
    ...
    const handleExportExcel = () => {
        const headers = ["Id", "Name", "Date", "Type", "Status", "Payee", "Account", "Category", "Notes", "Amount"];
        const csvContent = [
            headers.join(","),
            ...filteredTransactions.map(t => [
                ...
                t.transactionType?.name || t.txnType,
                Number(t.status) === 1 ? "Posted" : "Draft",
                `"${t.contact || t.payee || t.counterpartyName || ''}"`,
                `"${t.account?.name || ''}"`,
                `"${t.category?.name || ''}"`,
                `"${t.notes || ''}"`,
                t.amountBase
    ...
    const handleExportPDF = () => {
        setIsPrinting(true);
        // Allow React to re-render all rows before printing
        setTimeout(() => {
            window.print();
            setIsPrinting(false);
            setActiveDropdown(null);
    ...
    return (
        <div className="flex flex-col h-full min-h-0 overflow-hidden">
            ...
            <div className="flex-1 min-h-0 p-4 lg:p-8 animate-in fade-in duration-500 overflow-hidden flex items-start">
                <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden w-full max-h-full min-h-0 flex flex-col">
                    ...
                    {/* Toolbar */}
                    <div className="p-4 flex flex-wrap items-center justify-between gap-3 border-b border-gray-50 relative z-40 print:hidden min-h-[74px]">
                        {isMobileSearchOpen ? (
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        ...
                                                        if (txn.attachmentPath) {
                                                            const fullUrl = txn.attachmentPath.startsWith('http')
                                                                ? txn.attachmentPath
                                                                : `/api${txn.attachmentPath.startsWith('/') ? '' : '/'}${txn.attachmentPath}`;
                    ...
                    {/* Table (Desktop Only) */}
                    <div className="txn-print-surface relative hidden lg:block print:block flex-1 min-h-0 overflow-x-auto overflow-y-auto no-scrollbar txn-laptop-list-table-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} aria-busy={loading}>
                        <table className="w-full text-left border-collapse txn-laptop-list-table">
                            <thead className="sticky top-0 z-10 bg-white">
                                <tr className="bg-gray-50/50 border-y border-gray-200">
                                    {(() => {
                                        ...
                                        const baseCols = [
                                            { label: 'Id', key: 'id' },
                                            { label: 'Party', key: 'payee' },
                                            { label: 'Date', key: 'txnDate' },
                                            { label: 'Type', key: 'type' },
                                            { label: 'Account', key: 'account' },
                                            { label: 'Category', key: 'category' },
                                            { label: 'Notes', key: 'notes' },
                                            { label: 'Amount', key: 'amountBase' },
                                            { label: 'Created By', key: 'createdByName' }
                                        ];
                                        if (hasBranchColumn) {
                                            baseCols.splice(4, 0, { label: 'Branch', key: 'branch' });
                                        }
                                        return baseCols
                                            .map((col, i) => (
                                                <th
                                                    key={col.label}
                                                    className={cn(
                                                        "sticky top-0 z-10 px-4 py-2 text-[11px] font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap cursor-pointer hover:bg-gray-100/50 bg-gray-50/95 backdrop-blur-sm group print:bg-transparent print:border-black print:border",
                                                        col.label === 'Amount' ? "text-left" : "text-left",
                                                        col.label === 'Status' ? "text-center" : "",
                                                        col.label === 'Party' ? "max-w-[150px] truncate" : "",


                                                        col.label === 'Created By' ? cn("min-w-[100px] px-2", !(selectedBranch?.id === 'all' || selectedBranch?.id === 'multi') && "pl-6") : "",
                                                        col.label === 'Account' ? "px-2" : "",
                                                        (col.label === 'Date' || col.label === 'Notes') ? "px-2" : "",
                                                        col.label === 'Date' ? "min-w-[90px]" : "",
                                                        col.label === 'Type' ? "min-w-[80px]" : ""
                                                    )}
                                                    onClick={() => handleSort(col.key)}
                                                >
                                                    <div className={cn(
                                                        "flex items-center gap-1",
                                                        col.label === 'Amount' && "justify-start",
                                                        col.label === 'Status' && "justify-center"
                                                    )}>
                                                        <span>{col.label}</span>
                                                        <ArrowUpDown
                                                            size={10}
                                    ...
                                    <th className="px-2 py-2 bg-gray-50/95 backdrop-blur-sm print:hidden min-w-[110px] w-[110px] sticky top-0 right-0 z-20 shadow-[-5px_0_10px_-5px_rgba(0,0,0,0.1)] txn-laptop-action-col"></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {showInitialLoader ? (
                                    <tr><td colSpan={(selectedBranch?.id === 'all' || selectedBranch?.id === 'multi') ? 11 : 10} className="px-6 py-8 text-center text-sm text-gray-500">Loading...</td></tr>
                                ) : hasFetchedOnce && (isPrinting ? filteredTransactions : paginatedTransactions).length === 0 ? (
                                    <tr><td colSpan={(selectedBranch?.id === 'all' || selectedBranch?.id === 'multi') ? 11 : 10} className="px-6 py-8 text-center text-sm text-gray-500">No transactions found.</td></tr>
                                ) : (
                                    (isPrinting ? filteredTransactions : paginatedTransactions).map((txn, rowIndex) => (
                                        <tr key={txn.id} className="group hover:bg-gray-50/50">
                                            ...
                                            <td className="px-2 py-1.5 whitespace-nowrap overflow-visible">
                                                <PartyTooltip partyName={txn.contact || txn.payee || txn.counterpartyName} />
                                            </td>
                                            <td className="px-2 py-1.5 text-xs font-medium text-gray-700 whitespace-nowrap">
                                                {formatDate(txn.txnDate)}
                                            </td>
                                            <td className="px-4 py-1.5 border-none">
                                                <span className={cn(
                                            ...
                                            {(selectedBranch?.id === 'all' || selectedBranch?.id === 'multi') && (
                                                <td className="px-4 py-1.5 whitespace-nowrap overflow-visible">
                                                    <BranchTooltip branchNames={txn.branchNames} />
                                                </td>
                                            )}
                                            <td className="px-2 py-1.5 text-xs font-medium text-gray-600 whitespace-nowrap">
                                                {txn.accountName ||
                                                    (typeof txn.account === 'object' && txn.account !== null ? txn.account.name : txn.account) ||
                                                    txn.method ||
                                                    '-'}
                                            ...
                                            <td className={cn(
                                                "py-1.5 text-xs font-bold whitespace-nowrap",
                                                "text-left px-2"
                                            )}>
                                                <span className={cn(
                                                    "inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded-md",
                                                    Number(txn.amountBaseCurrency || txn.amountBase) >= 0 ? "text-emerald-600" : "text-rose-600",
                                                    txn.txnType === 'transfer' && "text-blue-600"
                                                )}>
                                                    {formatCurrency(txn.amountBaseCurrency || txn.finalAmountLocal || txn.amountBase, txn.baseCurrency)}
                                                </span>
                                            </td>
                                            <td className={cn(
                                                "py-1.5 text-xs font-medium text-gray-500 whitespace-nowrap min-w-[100px]",
                                                !(selectedBranch?.id === 'all' || selectedBranch?.id === 'multi') ? "px-2 pl-6" : "px-2"
                                            )}>
                                                {txn.createdByName || '-'}
                                            </td>
                            ...
                            <tfoot className="bg-gray-50/50 border-t border-gray-100">
                                <tr>
                                    <td colSpan={(selectedBranch?.id === 'all' || selectedBranch?.id === 'multi') ? 8 : 7} className="px-4 py-3 text-right text-[10px] font-bold py-2 print:border-none print:py-4 tracking-widest text-gray-900 border-none">
                                        TOTALS:
                                    </td>
                                    <td className="px-2 py-3 text-left text-[12px] font-black text-gray-900 print:border-none">
                                    ...
                                    {/* In print, we don't need the empty cell for created_by or actions in the totals row if we just use a small colspan */}
