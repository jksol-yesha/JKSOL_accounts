        ...
        const fetchAccounts = async () => {
            ...
            try {
                const [accountsResponse, rankingsResponse] = await Promise.all([
                    apiService.accounts.getAll({ branchId: 'all' }, { signal: controller.signal }),
                                                ...
                                                <div key={account.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm space-y-3 accounts-tablet-card">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-x-4 gap-y-3 accounts-tablet-card-grid">
                                                        ...
                                                        {(() => {
                                                            ...
                                                            return (
                                                                <div className="text-right">
                                                                    <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Owe Amount</div>
                                                                    {oweValue === 0 ? (
                    ...
                    <div className="relative print:block flex-1 min-h-0 overflow-x-auto overflow-y-auto no-scrollbar accounts-laptop-table-scroll" aria-busy={loading}>
                        <table className="w-full text-left border-collapse table-fixed accounts-laptop-table">
                                ...
                                <tr className="bg-gray-50/50 border-y border-gray-200">
                                    {(() => {
                                        const cols = [
                                            ...
                                            { label: 'Name', key: 'name', width: 'w-[14%]', align: 'text-left' },
                                            { label: 'Type', key: 'typeLabel', width: 'w-[12%]', align: 'text-left' },
                                            { label: 'SubType', key: 'subtypeLabel', width: 'w-[11%]', align: 'text-left' },
                                            { label: 'Opening Balance', key: 'openingBalance', width: 'w-[10%]', align: 'text-right' },
                                            { label: 'Net Settlement', key: 'oweAmount', width: 'w-[10%]', align: 'text-right' },
                                            { label: 'Date', key: 'openingBalanceDate', width: 'w-[14%]', align: 'text-center' },
                                            { label: 'Description', key: 'description', width: 'w-[12%]', align: 'text-left' },
                                            { label: 'Closing Balance', key: 'closingBalance', width: 'w-[10%]', align: 'text-right' },
                                            { label: 'Status', key: 'isActive', width: 'w-[9%]', align: 'text-left' },
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {showInitialLoader ? (
                                    <tr><td colSpan={tableColSpan} className="px-6 py-8 text-center text-sm text-gray-500">Loading...</td></tr>
                                        ...
                                        return (
                                            <React.Fragment key={account.id}>
                                                <tr className="group hover:bg-gray-50/50">
                                                    ...
                                                    <td className="px-4 py-1.5 pl-12 text-xs font-black tabular-nums text-right whitespace-nowrap">
                                                        {(() => {
                                                            let oweValue = getAccountOweValue(account);
                                                    ...
                                                    <td className="px-4 py-1.5 text-xs font-black tabular-nums text-right whitespace-nowrap">
                                                        <span className={cn(
                                                            "text-xs font-black",
                                                            getDisplayClosingBalance(account) > 0 ? "text-emerald-600" : getDisplayClosingBalance(account) < 0 ? "text-rose-600" : "text-gray-600"
                                                        )}>
                                                            ...
                                                            <div className="grid grid-cols-4 gap-4 pl-12 text-[11px]">
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="font-bold text-gray-500 uppercase">AccountNo:</span>{' '}
                                                                        <span>{account.accountNumber?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="font-bold text-gray-500 uppercase">IFSCCode:</span>{' '}
                                                                        <span>{account.ifsc?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="font-bold text-gray-500 uppercase">SwiftCode:</span>{' '}
                                                                        <span>{account.swiftCode?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="font-bold text-gray-500 uppercase">BranchName:</span>{' '}
                                                                        <span>{account.bankBranchName?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </td>
