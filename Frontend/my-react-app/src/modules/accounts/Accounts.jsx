...
const isAssetAccount = (account) => {
    return Number(account?.type || account?.accountType) === 1 ||
        account?.accountType === 'Asset' ||
        account?.typeLabel === 'Asset' ||
        account?.typeLabel === 'ASSET';
};
...
const Accounts = () => {
    ...
    const [pageSize, setPageSize] = useState(20); // Show 20 rows per page as requested
    ...
    const getAccountOweValue = (account) => {
        const keys = Array.from(new Set([
            normalizeAccountKey(account?.name),
            normalizeAccountKey(account?.accountName),
            normalizeAccountKey(account?.bankName)
        ].filter(Boolean)));

        return keys.reduce((sum, key) => sum + (oweMap.get(key) || 0), 0);
    };
        ...
        const fetchAccounts = async () => {
            ...
            try {
                ...
                if (rankingsResponse.success && rankingsResponse.data) {
                    ...
                    // Build a set of investment account keys to exclude them from net settlement
                    const investmentAccountKeys = new Set(
                        fetchedAccounts
                            .filter(a => Number(a.accountType) === 1 && Number(a.subtype) === 14)
                            .flatMap(a => [
                                normalizeAccountKey(a.name),
                                normalizeAccountKey(a.accountName),
                                normalizeAccountKey(a.bankName)
                            ].filter(Boolean))
                    );

                    const applyDelta = (key, delta) => {
                        if (!key) return;
                        newOweMap.set(key, (newOweMap.get(key) || 0) + delta);
                    };
                    ...
                    transfers.forEach(t => {
                        ...
                        const owingKey = normalizeAccountKey(t.owingAccount || t.owingAccountName || t.fromAccount || t.fromAccountName);
                        const owedKey = normalizeAccountKey(t.owedAccount || t.owedAccountName || t.toAccount || t.toAccountName);

                        // Skip if it's explicitly an investment transaction or involves an investment account
                        if (isInvestmentTxn || investmentAccountKeys.has(owingKey) || investmentAccountKeys.has(owedKey)) {
                            return;
                        }

                        const amount = parseTransferAmount(t.amount ?? t.value ?? t.totalAmount);

                        if (!owingKey || !owedKey || amount === 0) return;

                        // `owingKey` is the account that owes the money (it received the transfer and now must PAY it back) -> negative (-)
                        // `owedKey` is the account that is owed the money (it sent the transfer and will RECEIVE it back) -> positive (+)
                        applyDelta(owingKey, -amount);
                        applyDelta(owedKey, +amount);
                    });
                    setOweMap(newOweMap);
                    try {
    ...
    }, [cacheKey, preferences.currency, selectedYear?.id]);
    ...
    useEffect(() => {
        ...
        const unsubscribeCreate = on('account:created', (newAccount) => {
            // console.log('🔥 New account received:', newAccount);
        ...
        const unsubscribeUpdate = on('account:updated', (updatedAccount) => {
            // console.log('✏️ Account updated:', updatedAccount);
    ...
    const filteredAccounts = useMemo(() => {
        ...
        if (sortConfig.key) {
            result.sort((a, b) => {
                ...
                } else if (sortConfig.key === 'oweAmount') {
                    aValue = getAccountOweValue(a);
                    bValue = getAccountOweValue(b);
    ...
    const handleToggleStatus = async (id, currentIsActive) => {
        try {
            ...
            const response = await apiService.accounts.getAll({ branchId: 'all', currency: preferences.currency });
            if (response.success) {
        ...
        <div className="accounts-tablet-page flex flex-col h-full min-h-0 overflow-hidden">
            <style>{`
                @media print {
                    ...
                    /* Center Align Opening Balance (5th Column) */
                    th:nth-child(5), td:nth-child(5) { text-align: center !important; }
                    th:nth-child(5) > div { justify-content: center !important; }
                }
                                                    ...
                                                    <div className="grid grid-cols-2 gap-x-4 gap-y-3 accounts-tablet-card-grid">
                                                        <div>
                                                            <div>
                                                                ...
                                                                <div className="font-medium text-gray-600 text-xs">{account.typeLabel || '-'}</div>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Subtype</div>
                                                            <div className="font-medium text-gray-600 text-xs">{account.subtypeLabel || '-'}</div>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Date</div>
                                                            <div className="font-medium text-gray-600 text-xs">{formatDate(account.openingBalanceDate)}</div>
                                                        </div>
                                                        <div>
                                                            <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Opening Balance</div>
                                                            <div className="font-bold text-xs text-emerald-600 tabular-nums whitespace-nowrap">
                                                                {formatCurrency(getDisplayBalance(account), account.baseCurrency)}
                                                            </div>
                                                        </div>
                                                        {(() => {
                                                            let oweValue = getAccountOweValue(account);
                                                            return (
                                                                <div className="text-right">
                                                                    <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Net Settlement</div>
                                                                    {oweValue === 0 ? (
                                                                        <div className="font-medium text-gray-400 text-xs">-</div>
                                                                    ) : (
                                                                        <div className={cn(
                                                                            "font-bold text-xs tabular-nums whitespace-nowrap",
                                                                            oweValue > 0 ? "text-emerald-600" : "text-rose-600"
                                                                        )}>
                                                                            {oweValue > 0 ? '+' : '-'}{formatCurrency(Math.abs(oweValue), account.baseCurrency)}
                                                                        </div>
                                                                    )}
                                                                </div>
                                                            );
                                                        ...
                                                        <div className="col-span-2 pt-1">
                                                            <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5 text-left">Closing Balance</div>
                                                            <div className={cn(
                                                                "font-bold text-xs tabular-nums whitespace-nowrap text-left",
                                                        ...
                                                        {hasDescription && (
                                                            <div className="col-span-1 md:col-span-2">
                                                                ...
                                                                <div className="font-medium text-gray-600 text-xs break-words">{account.description}</div>
                                                            </div>
                                                        )}
                                                        <div>
                                                            <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Status</div>
                                                            <button
                                                        ...
                                                        <div className="text-right">
                                                            <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Created By</div>
                                                            <div className="font-medium text-gray-600 text-xs">{account.createdByDisplayName || '-'}</div>
                                                        </div>
                                                        {hasAccountNumber && (
                                                            <div>
                                                                <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Account No</div>
                                                                <div className="font-medium text-gray-600 text-xs tabular-nums break-all">{account.accountNumber}</div>
                                                            </div>
                                                        )}

                                                        {hasIfsc && (
                                                            <div className="md:text-right">
                                                                <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">IFSC Code</div>
                                                                <div className="font-medium text-gray-600 text-xs">{account.ifsc}</div>
                                                            </div>
                                                        )}
                                                        {isAssetBankAccount(account) && (
                                                            <>
                                                                {hasSwiftCode && (
                                                                    <div>
                                                                        <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Swift Code</div>
                                                                        <div className="font-medium text-gray-600 text-xs">{account.swiftCode}</div>
                                                                    </div>
                                                                )}
                                                                {hasBankBranchName && (
                                                                    <div className="md:text-right">
                                                                        <div className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-0.5">Branch Name</div>
                                                                        <div className="font-medium text-gray-600 text-xs">{account.bankBranchName}</div>
                                                                    </div>
                                                                )}
                                                            </>
                                                        )}
                        ...
                        <table className="w-full text-left border-collapse table-fixed accounts-laptop-table">
                            <thead className="sticky top-0 z-10 bg-white">
                                <tr className="bg-gray-50/50 border-y border-gray-200">
                                    {(() => {
                                        const cols = [
                                            { label: 'Id', key: 'syntheticId', width: 'w-[5%]', align: 'text-left' },
                                            { label: 'Name', key: 'name', width: 'w-[12%]', align: 'text-left' },
                                            { label: 'Type', key: 'typeLabel', width: 'w-[8%]', align: 'text-left' },
                                            { label: 'SubType', key: 'subtypeLabel', width: 'w-[8%]', align: 'text-left' },
                                            { label: 'Date', key: 'openingBalanceDate', width: 'w-[9%]', align: 'text-center' },
                                            { label: 'Opening Balance', key: 'openingBalance', width: 'w-[12%]', align: 'text-right' },
                                            { label: 'Net Settlement', key: 'oweAmount', width: 'w-[12%]', align: 'text-right' },
                                            { label: 'Closing Balance', key: 'closingBalance', width: 'w-[12%]', align: 'text-right' },
                                            { label: 'Description', key: 'description', width: 'w-[15%]', align: 'text-left' },
                                            { label: 'Status', key: 'isActive', width: 'w-[8%]', align: 'text-left' },
                                            { label: 'Created By', key: 'creatorName', width: 'w-[11%]', align: 'text-left' }
                                        ];
                                        return cols.map((col) => (
                                            <th
                                                key={col.label}
                                                className={cn(
                                                    ...
                                                    col.key === 'openingBalance' && "pr-5",
                                                    col.key === 'oweAmount' && "pl-6 pr-0"
                                                )}
                                            ...
                                            >
                                                <div className={cn(
                                                    ...
                                                    (col.key === 'openingBalance' || col.key === 'closingBalance' || col.key === 'oweAmount') && "accounts-laptop-sort-header"
                                                )}>
                                                    {col.key === 'syntheticId' && <span className="w-[14px] inline-block" />}
                                                    <span className={cn(
                                                        (col.key === 'openingBalance' || col.key === 'closingBalance') && "accounts-laptop-balance-label",
                                                        col.key === 'oweAmount' && "accounts-laptop-owe-label"
                                                    )}>
                                                        {col.label}
                                                    </span>
                                                    <ArrowUpDown
                                                        size={(col.key === 'openingBalance' || col.key === 'closingBalance' || col.key === 'oweAmount') ? 12 : 10}
                                                        className={cn(
                                                            ...
                                                            (col.key === 'openingBalance' || col.key === 'closingBalance' || col.key === 'oweAmount') && "accounts-laptop-sort-icon",
                                                            col.key === 'openingBalance' && "accounts-laptop-opening-sort-icon"
                                                        )}
                                                    />
                                                </div>
                                            </th>
                                    ...
                                    <th className="w-[7%] sticky top-0 z-10 bg-gray-50/95 backdrop-blur-sm px-4 py-2 text-right text-[11px] font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">Action</th>
                                </tr>
                            </thead>
                                            ...
                                            <React.Fragment key={account.id}>
                                                <tr className="group hover:bg-gray-50/50">
                                                    <td className="px-4 py-1.5 text-xs text-gray-500 font-mono">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span>{syntheticId}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-1.5 text-xs font-bold text-gray-800">
                                                        <span className="text-xs font-bold text-gray-800 text-left cursor-default">
                                                            {account.name}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-1.5 whitespace-nowrap">
                                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200 uppercase tracking-wide">
                                                            {account.typeLabel || 'Unknown'}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-1.5 whitespace-nowrap">
                                                    ...
                                                    <td className="px-4 py-1.5 whitespace-nowrap text-center text-xs text-gray-600 font-medium">
                                                        {formatDate(account.openingBalanceDate)}
                                                    </td>
                                                    <td className="px-4 py-1.5 pr-5 text-xs font-black tabular-nums text-right whitespace-nowrap">
                                                    ...
                                                    <td className="px-4 py-1.5 pl-9 pr-6 text-xs font-black tabular-nums text-right whitespace-nowrap">
                                                        {(() => {
                                                            let oweValue = getAccountOweValue(account);
                                                            if (oweValue === 0) return <span className="text-gray-400 font-medium">-</span>;
                                                    ...
                                                    <td className="px-4 py-1.5 pl-7 text-xs font-black tabular-nums text-right whitespace-nowrap">
                                                        <span className={cn(
                                                            "text-xs font-black",
                                                            getDisplayClosingBalance(account) > 0 ? "text-emerald-600" : getDisplayClosingBalance(account) < 0 ? "text-rose-600" : "text-gray-600"
                                                        )}>
                                                    ...
                                                    <td className="px-4 py-1.5 whitespace-nowrap overflow-visible">
                                                        <DescriptionTooltip description={account.description} />
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-1.5 text-right print:hidden">
                                                ...
                                                {showExpand && isExpanded && (
                                                    <tr className="bg-gray-50/60">
                                                        <td colSpan={tableColSpan} className="px-8 py-2">
                                                            <div className="grid grid-cols-4 gap-4 pl-12 text-[11px]">
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="text-gray-500 uppercase">AccountNo:</span>{' '}
                                                                        <span className="font-bold">{account.accountNumber?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="text-gray-500 uppercase">IFSCCode:</span>{' '}
                                                                        <span className="font-bold">{account.ifsc?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="text-gray-500 uppercase">SwiftCode:</span>{' '}
                                                                        <span className="font-bold">{account.swiftCode?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                    <div className="text-gray-700 font-medium">
                                                                        <span className="text-gray-500 uppercase">BranchName:</span>{' '}
                                                                        <span className="font-bold">{account.bankBranchName?.trim() || 'N/A'}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </td>
