...
const Parties = () => {
    ...
    const CACHE_KEY = `parties_cache_${selectedOrg?.id}_${selectedBranch?.id || 'all'}`;
    ...
    useEffect(() => {
        ...
        const fetchParties = async () => {
            ...
            try {
                ...
                const branchFilter = 'all'; // Bypass global filter
                ...
                if (isMultiBranch) {
                    const responses = await Promise.all(
                        ['all'].map(bId => apiService.parties.getAll(
                            { branchId: bId },
                            { signal: controller.signal, headers: { 'x-org-id': selectedOrg.id } }
                        ))
                    );
                    responses.forEach((res, index) => {
                        const branchParties = (res && Array.isArray(res.data)) ? res.data : (Array.isArray(res) ? res : []);
                        const targetBId = 'all';
                        const fallbackName = branches.find(b => String(b.id) === targetBId)?.name || 'Unknown Branch';
                        branchParties.forEach(p => {
                            p._sourceBranchId = p.branchId || targetBId;
                            p._sourceBranchName = p.branch?.name || branches.find(b => String(b.id) === String(p.branchId))?.name || fallbackName;
                        });
                        allParties.push(...branchParties);
                    });
                } else {
                    ...
                    let branchParties = (response && Array.isArray(response.data)) ? response.data : (Array.isArray(response) ? response : []);
                    ...
                    allParties = branchParties;
                }

                // Group parties by name
                const groupedPartiesMap = new Map();
                allParties.forEach(p => {
                    const nameKey = (p.name || '').toLowerCase().trim();
                    if (!groupedPartiesMap.has(nameKey)) {
                        groupedPartiesMap.set(nameKey, {
                            ...p,
                            branchNames: [p._sourceBranchName],
                            branchIds: [p._sourceBranchId]
                        });
                    } else {
                        const existing = groupedPartiesMap.get(nameKey);
                        if (!existing.branchIds.includes(p._sourceBranchId)) {
                            existing.branchNames.push(p._sourceBranchName);
                            existing.branchIds.push(p._sourceBranchId);
                        }
                    }
                });

                const finalParties = Array.from(groupedPartiesMap.values()).map(({ _sourceBranchId, _sourceBranchName, ...rest }) => rest);
    ...
    }, [selectedBranch?.id, selectedOrg?.id, getBranchFilterValue]);
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this party?')) {
            try {
                await apiService.parties.delete(id);
                            ...
                            <div key={party.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm space-y-3">
                                <div className="flex items-start justify-between gap-3 border-b border-gray-50 pb-2">
                                    <div>
                                        ...
                                        <div className="text-sm font-bold text-gray-800">{party.name || '-'}</div>
                                    </div>
                                    <button
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {true && (
                                        <div className="col-span-2">
                                            ...
                                            <div className="text-xs font-medium text-gray-600">{(party.branchNames || []).join(', ') || '-'}</div>
                                        </div>
                                    )}
                                        ...
                                        <button
                                            onClick={() => {
                                                navigate('/parties/create', {
                                                    state: {
                                                        party,
                                                        siblingParties: parties.filter(
                                                            p => p.name?.toLowerCase().trim() === party.name?.toLowerCase().trim()
                                                        ),
                                                        originalBranchIds: party.branchIds || [party.branchId]
                                                    }
                                                });
                                            }}
                            ...
                            <tr className="bg-gray-50/50 border-y border-gray-200">
                                {(() => {
                                    const baseCols = [
                                        ...
                                        { label: 'Email', key: 'email', width: 'w-[17%]' },
                                        { label: 'Address', key: 'address', width: 'w-[16%]' },
                                        { label: 'GST No', key: 'gstNo', width: 'w-[12%]' },
                                        { label: 'GST Name', key: 'gstName', width: 'w-[13%]' },
                                ...
                                <th className="w-[6%] pl-6 pr-4 py-2 text-right text-[11px] font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">Action</th>
                            </tr>
                        </thead>
                                ...
                                paginatedParties.map((party, index) => (
                                    <tr key={party.id} className="group hover:bg-gray-50/50">
                                        ...
                                        <td className="px-4 py-1.5 text-xs font-bold text-gray-800">
                                            <span className="text-xs font-bold text-gray-800">{party.name}</span>
                                        </td>
                                        <td className="px-4 py-1.5 whitespace-nowrap">
                                        ...
                                        <td className="pl-3 pr-2 py-1.5 whitespace-nowrap">
                                            <span className="text-xs font-medium text-gray-600">{party.gstName || '-'}</span>
                                        </td>
                                        ...
                                        <td className="pl-6 pr-4 py-1.5 text-right print:hidden">
