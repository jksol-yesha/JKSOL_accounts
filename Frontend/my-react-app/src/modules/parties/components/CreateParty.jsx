...
const CreateParty = () => {
    ...
    // Build a map of branchId -> existing party record for editing siblings
    const siblingParties = location.state?.siblingParties || [];
    const siblingMap = new Map(); // branchId -> party
    siblingParties.forEach(p => {
        const bIds = p.branchIds || [p.branchId];
        bIds.forEach(bId => siblingMap.set(Number(bId), p));
    });
    const originalBranchIds = (location.state?.originalBranchIds || []).map(Number).filter(Boolean);

    // Target Branch state (always global default)
    const [targetBranchIds, setTargetBranchIds] = useState(() => {
        if (editingParty && originalBranchIds.length > 0) return originalBranchIds;
    ...
    const validateField = (name, value) => {
        ...
        switch (name) {
            case 'name':
                if (!value.trim()) error = 'Contact Name is compulsory.';
                break;
            case 'email':
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing) {
                // Multi-branch edit: for each selected branch, fetch the branch-specific
                // party record by name to get the correct per-branch ID, then update it.
                const originalName = editingParty.name?.toLowerCase().trim();

                for (const bId of targetBranchIds) {
                    const isExistingBranch = originalBranchIds.includes(Number(bId));

                    if (isExistingBranch) {
                        // Find the real ID of this party in that branch
                        const res = await apiService.parties.getAll({ branchId: Number(bId) });
                        const branchParties = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
                        const match = branchParties.find(p => p.name?.toLowerCase().trim() === originalName);
                        if (match) {
                            await apiService.parties.update(match.id, { ...basePayload, branchId: Number(bId) });
                        }
                    } else {
                        // Newly added branch — create the party there
                        await apiService.parties.create(basePayload, { branchId: Number(bId) });
                    }
                }

                // Remove party from branches that were DE-selected
                const removedBranchIds = originalBranchIds.filter(bId => !targetBranchIds.map(Number).includes(Number(bId)));
                for (const bId of removedBranchIds) {
                    const res = await apiService.parties.getAll({ branchId: Number(bId) });
                    const branchParties = Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : []);
                    const match = branchParties.find(p => p.name?.toLowerCase().trim() === originalName);
                    if (match) {
                        await apiService.parties.delete(match.id);
                    }
                }

                addToast?.('Party updated successfully', 'success');
            } else {
                // Create mode — always create in all branches
                const createBranchIds = (branches || []).map(b => Number(b.id)).filter(Boolean);
                if (createBranchIds.length === 0) {
                    throw new Error('No branches found to create party');
                }
                for (const bId of createBranchIds) {
                    await apiService.parties.create(basePayload, { branchId: bId });
                }
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-50/50">
            ...
            <div className="p-4 lg:p-8 max-w-2xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                <form onSubmit={handleSubmit}>
                    <div className="bg-white rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border border-gray-100 p-6 md:p-10 space-y-4">
                        ...
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h2 className="text-lg font-bold text-gray-900">{isEditing ? "Edit Party" : "Add New Party"}</h2>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className={`relative flex items-stretch bg-gray-50 border ${errors.phone ? 'border-rose-500' : 'border-gray-50 focus-within:border-black'} rounded-xl transition-all h-[40px]`} ref={dropdownRef}>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center justify-start gap-2">
                                <h3 className="text-xs font-bold text-gray-900">Taxable</h3>
