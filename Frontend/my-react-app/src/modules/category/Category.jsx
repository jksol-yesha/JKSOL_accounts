...
const Category = () => {
    const { selectedBranch, loading: branchLoading, branches } = useBranch();
    ...
    const getCategoryIdentityKey = (category = {}) => (
        `${normalizeCategoryName(category?.name)}::${normalizeCategoryType(getCategoryTypeValue(category))}`
    );

    const matchesCategoryIdentity = (leftCategory, rightCategory) => (
        getCategoryIdentityKey(leftCategory) === getCategoryIdentityKey(rightCategory)
    );

    const getSubCategoryIdentityKey = ({ parentName = '', parentType = '', name = '' }) => (
        `${normalizeCategoryName(parentName)}::${normalizeCategoryType(parentType)}::${normalizeCategoryName(name)}`
    );
