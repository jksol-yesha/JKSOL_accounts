...
const EditCategoryModal = ({ isOpen, onClose, category, onSave }) => {
    const [formData, setFormData] = useState({ name: '', type: 'Expense', icon: '', status: 1, pnlClassification: '' });
    ...
    useEffect(() => {
        if (category) {
            setFormData({
                ...
                pnlClassification: category.pnlClassification || ''
            });
            setNameError('');
        }
    }, [category]);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setNameError('');
        const originalType = category.txnType
            ? (category.txnType.charAt(0).toUpperCase() + category.txnType.slice(1))
                        ...
                        <div className="space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <CustomSelect
                                        value={formData.type}
                                        onChange={(e) => setFormData({ ...formData, type: e.target.value, pnlClassification: '' })}
