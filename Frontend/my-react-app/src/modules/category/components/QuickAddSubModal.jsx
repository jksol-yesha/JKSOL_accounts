...
const QuickAddSubModal = ({ isOpen, onClose, parentCategory, onSave }) => {
    ...
    const parentLabel = parentCategory?.type ? `${parentCategory.name} (${parentCategory.type})` : parentCategory?.name;
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300" onClick={handleClose} />
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    <h2 className="text-[17px] font-extrabold text-slate-800 tracking-tight flex items-baseline">
                        Quick Add Sub to: <span className="ml-2 text-emerald-600">{parentLabel}</span>
                    </h2>
                    <button
                        onClick={handleClose}
