...
export const authMiddleware = (app: Elysia) => app
    ...
    .derive(async ({ jwt, headers, set, request }) => {
        ...
        // DEBUG: Explicitly log start of Auth Middleware
        console.log(`🔐 [AuthMiddleware] Processing request. Authorization present: ${!!authorization}`);


        console.log('🔐 Auth Middleware - Headers:', { authorization: authorization ? 'Present' : 'Missing', orgIdHeader, branchIdHeader });

        if (!authorization) {
            console.log('❌ Auth Middleware - No authorization header'); // Changed to log instead of error to avoid noise for public endpoints if any
            // set.status = 401; // Don't set status here, just return null user. Let the route decide if it needs user.
            return {
                user: null,
                orgId: null,
                branchId: null,
                authError: 'No authorization header'
            };
        }
        ...
        try {
            ...
            console.log(`🔐 [AuthMiddleware] Payload Success - Sub: ${payload.sub}, Role: ${payload.role}`);
            ...
            console.log(`✅ [AuthMiddleware] Auth Success: ${user.email} (${user.role})`);

            console.log('✅ Auth Middleware - User found:', { id: user.id, email: user.email, role: user.role, orgIds: user.orgIds, branchIds: user.branchIds });
