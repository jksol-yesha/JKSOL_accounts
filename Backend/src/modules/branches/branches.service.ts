import { db } from '../../db';
import { branches, organizations, accounts, transactions, monthlyBranchSummary, yearlyBranchSummary, categories, users, roles } from '../../db/schema';
...
export const createBranch = async (data: {
    ...
    // If code is provided, check for uniqueness separately
    if (data.code) {
        ...
        if (existingCode) {
            throw new Error(`Branch with code '${data.code}' already exists in this organization.`);
        }
    }
    ...
    const result = await db.insert(branches).values({
        ...
        defaultGstRate: data.defaultGstRate?.toString() || '0',
...
export const updateBranch = async (id: number, data: {
    ...
    if (updateData.code === '' || updateData.code === undefined) {
        // Only convert to null if it's explicitly an empty string
        // If it's undefined, we don't want to change it (it's an optional update field)
        if (updateData.code === '') updateData.code = null as any;
    }
    if (data.defaultGstRate !== undefined) {
        updateData.defaultGstRate = data.defaultGstRate.toString() as any;
    }
...
export const deleteBranch = async (id: number, userId: number) => {
    ...
    return await db.transaction(async (tx) => {
        ...
        // C. Delete Accounts linked to this branch
        await tx.delete(accounts).where(eq(accounts.branchId, id));

        // D. Delete Categories linked to this branch
        await tx.delete(categories).where(eq(categories.branchId, id));
