...
export const createTransaction = async ({ body, set, user, orgId, branchId }: ElysiaContext & { body: any }) => {
    ...
    try {
        ...
        const effectiveBranchId = (branchId === 'all') ? (data.branchId ? Number(data.branchId) : null) : (branchId || (data.branchId ? Number(data.branchId) : null));
        ...
        console.log("Create Transaction Incoming Data:", JSON.stringify(data, null, 2));
...
export const updateTransaction = async ({ params, body, set, user, orgId }: ElysiaContext & { params: { id: string }, body: any }) => {
    ...
    try {
        ...
        const parseId = (val: any) => {
            if (val === undefined || val === null || val === '' || val === 'null' || val === 'undefined') return undefined; // undefined to not overwrite existing with null
            const num = Number(val);
            return isNaN(num) || num === 0 ? null : num;
        };
...
export const importFromPDF = async ({ body, set, user, orgId }: ElysiaContext & { body: { file: File, accountId?: string, branchId?: string, financialYearId?: string } }) => {
    if (!user || !orgId) throw new Error("Unauthorized: User or Organization not identified");
    try {
        console.log('PDF Import - Received body:', {
            hasFile: !!body.file,
            accountId: body.accountId,
            branchId: body.branchId,
            financialYearId: body.financialYearId,
            accountIdType: typeof body.accountId,
            branchIdType: typeof body.branchId
        });
...
export const getTransactions = async ({ body, set, user, orgId, branchId, headers }: ElysiaContext & { body: any & { branchId?: number | number[] | 'all' } }) => {
    console.log(`[Transactions] getTransactions - ContextBranchId: ${branchId}, BodyBranchId: ${body.branchId}`);
    if (!user || !orgId) throw new Error("Unauthorized: User or Organization not identified");
    try {
        const financialYearId = Number(body.financialYearId);
        const limit = body.limit ? Number(body.limit) : undefined;
        const rawBranchId = body.branchId ?? branchId;
        console.log(`[Transactions] getTransactions - RawBranchId: ${rawBranchId}, Context: ${branchId}, Body: ${body.branchId}`);
