...
export class TransactionService {
    ...
    static async importTransactions(buffer: Buffer, orgId: number, user: any, defaultFinancialYearId?: number, defaultBranchId?: number, autoGenerate: boolean = false) {
        ...
        for (let i = 0; i < normalizedRows.length; i++) {
            ...
            if (rowErrors.length > 0) {
                ...
                validTransactions.push({
                    ...
                    referenceNo: String(row.reference || row.referenceno || ''),
        ...
        console.log(`[IMPORT DEBUG] Total Rows: ${rows.length}, Valid: ${validTransactions.length}, Validation Errors: ${errors.length}`);
        ...
        console.log(`[IMPORT RESULT] Success: ${successCount}, Failed: ${errors.length}`);
    ...
    static async getAll(orgId: number, branchId: number | number[] | 'all' | null, financialYearId: number, limit?: number, targetCurrency?: string, user?: any) {
        ...
        const txns = await db.select({
            ...
            createdActionAt: sql<string | null>`(
                SELECT DATE_FORMAT(a.action_at, '%Y-%m-%d %H:%i:%s')
                FROM audit_logs a
                WHERE a.org_id = ${orgId}
                  AND lower(a.entity) = 'transaction'
