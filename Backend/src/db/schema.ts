...
export const accounts = mysqlTable("accounts", {
  ...
  return {
    ...
    idxAccOrg: index("idx_acc_org").on(table.orgId),
...
export const categories = mysqlTable("categories", {
  ...
  status: int("status").notNull().default(1),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  pnlClassification: varchar("pnl_classification", { length: 50 }),
}, (table) => {
  return {
    ukCatOrgTypeName: uniqueIndex("uk_cat_org_type_name").on(table.orgId, table.txnTypeId, table.name),
...
export const transactions = mysqlTable("transactions", {
  ...
  contactId: bigint("contact_id", { mode: "number", unsigned: true }).references(() => parties.id),
...
export const transactionEntries = mysqlTable("transaction_entries", {
  ...
  accountId: bigint("account_id", { mode: "number", unsigned: true }).notNull().references(() => accounts.id),
...
export const organizationInvitations = mysqlTable("organization_invitations", {
  ...
  branchIds: varchar("branch_ids", { length: 2500 }).notNull().default(""),
