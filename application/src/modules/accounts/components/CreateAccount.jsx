...
const CreateAccount = () => {
    ...
    useEffect(() => {
        ...
        const scheduleMeasurement = () => {
            ...
            frameId = window.requestAnimationFrame(() => {
                ...
                window.dispatchEvent(new CustomEvent(ACCOUNTS_CREATE_SCROLL_MODE_EVENT, {
    ...
    const validateField = (name, value, currentFormData = formData) => {
        ...
        switch (name) {
            ...
            case 'accountHolderName':
                ...
                else if (String(value || '').trim().length > 150) error = 'Maximum 150 characters allowed';
                break;
            case 'bankName':
                if (
                    Number(currentFormData.accountType) === ACCOUNT_TYPES.ASSET &&
    ...
    const handleSubmit = async (e) => {
        ...
        const keysToValidate = isAssetBank
            ...
            : Object.keys(formData).filter(k => !['accountNumber', 'accountHolderName', 'bankName', 'ifsc', 'swiftCode', 'bankBranchName'].includes(k));
    ...
    return (
        ...
        >
            ...
            <div
                ref={shellRef}
                className={cn(
                    "accounts-create-shell p-4 lg:p-8 max-w-2xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500",
                    needsWholePageScroll && "accounts-create-shell-scroll"
                )}
            >
                <form onSubmit={handleSubmit} className="accounts-create-form">
                    <div ref={cardRef} className="accounts-create-card bg-white rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border border-gray-100 p-6 md:p-10 space-y-4">
                            ...
                            <div className="accounts-create-grid grid grid-cols-1 lg:grid-cols-2 gap-4">
                                <div className="accounts-create-field space-y-1">
                                    ...
                                    <input
                                        ...
                                        onChange={handleChange}
                                        onKeyDown={(e) => handleFieldKeyDown(e, 'accountHolderName')}
                                        placeholder="Enter account holder name"
                                        className={`w-full px-4 py-2 bg-gray-50 border ${errors.accountHolderName ? 'border-rose-500' : 'border-gray-50 focus:border-black'} rounded-xl text-[14px] font-bold text-slate-700 outline-none transition-all`}
                                    />
                                    {errors.accountHolderName && <p className="text-[10px] font-bold text-rose-500 mt-0.5 pl-1">{errors.accountHolderName}</p>}
                                </div>
                                ...
                                <div className="accounts-create-field space-y-1">
                                    ...
                                    <input
                                        ...
                                        className={`w-full px-4 py-2 bg-gray-50 border ${errors.bankName ? 'border-rose-500' : 'border-gray-50 focus:border-black'} rounded-xl text-[14px] font-bold text-slate-700 outline-none transition-all`}
                                    />
                                    {errors.bankName && <p className="text-[10px] font-bold text-rose-500 mt-0.5 pl-1">{errors.bankName}</p>}
                                </div>
