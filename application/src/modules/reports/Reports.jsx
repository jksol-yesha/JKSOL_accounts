...
const normalizeCategoryFilterValues = (value) => {
    const rawValues = Array.isArray(value)
        ? value
...
const isAllCategoryFilterSelected = (value, options = []) => {
    if (value === ALL_CATEGORIES_VALUE) return true;

    const normalized = normalizeCategoryFilterValues(value);
    return normalized.length === 0 || (options.length > 0 && normalized.length >= options.length);
};

const buildCategoryFilterValue = (values, options = []) => {
    const normalized = normalizeCategoryFilterValues(values);
...
const resolveCategoryFilterIds = (value, availableCategories = [], { allowMultiple = false } = {}) => {
    const selectedNames = normalizeCategoryFilterValues(value);
    ...
    selectedNames.forEach((selectedName) => {
        const match = availableCategories.find((category) => (
            String(category?.name || '').trim().toLowerCase() === selectedName.toLowerCase()
        ));

        if (!match?.id) return;

        const id = Number(match.id);
        if (!Number.isFinite(id) || id <= 0 || matchedIds.includes(id)) return;
        matchedIds.push(id);
    });

    if (matchedIds.length === 0) return undefined;
    if (!allowMultiple) return matchedIds[0];

    return matchedIds.length === 1 ? matchedIds[0] : matchedIds;
};

const doesReportTypeAllowMultipleCategories = (reportType) => (
    reportType === 'Detailed' || reportType === 'Category-wise'
);

const normalizeAccountFilterValues = (value) => {
    const rawValues = Array.isArray(value)
        ? value
...
const isAllAccountFilterSelected = (value, options = []) => {
    if (value === ALL_ACCOUNTS_VALUE) return true;

    const normalized = normalizeAccountFilterValues(value);
    return normalized.length === 0 || (options.length > 0 && normalized.length >= options.length);
};

const buildAccountFilterValue = (values, options = []) => {
    const normalized = normalizeAccountFilterValues(values);
...
const resolveAccountFilterIds = (value, availableAccounts = [], { allowMultiple = false } = {}) => {
    const selectedNames = normalizeAccountFilterValues(value);
    if (selectedNames.length === 0) return undefined;

    const matchedIds = [];

    selectedNames.forEach((selectedName) => {
        const match = availableAccounts.find((account) => {
            const label = String(account?.bankName || account?.name || '').trim().toLowerCase();
            return label === selectedName.toLowerCase();
        });
...
const doesReportTypeAllowMultipleAccounts = (reportType) => (
    reportType === 'Account-wise'
);

const normalizePartyFilterValues = (value) => {
    const rawValues = Array.isArray(value)
        ? value
        : value && value !== ALL_PARTIES_VALUE
            ? [value]
            : [];
...
const isAllPartyFilterSelected = (value, options = []) => {
    if (value === ALL_PARTIES_VALUE) return true;

    const normalized = normalizePartyFilterValues(value);
    return normalized.length === 0 || (options.length > 0 && normalized.length >= options.length);
};

const buildPartyFilterValue = (values, options = []) => {
    const normalized = normalizePartyFilterValues(values);

    if (normalized.length === 0 || (options.length > 0 && normalized.length >= options.length)) {
        return ALL_PARTIES_VALUE;
    }

    return normalized.length === 1 ? normalized[0] : normalized;
};

const resolvePartyFilterValue = (value, { allowMultiple = false } = {}) => {
    const selectedNames = normalizePartyFilterValues(value);
    if (selectedNames.length === 0) return undefined;
    if (!allowMultiple) return selectedNames[0];

    return selectedNames.length === 1 ? selectedNames[0] : selectedNames;
};

const doesReportTypeAllowMultipleParties = (reportType) => (
    reportType === 'Party-wise'
);
...
const ReportsTypeMultiSelect = React.forwardRef(({ value, options, onChange, onKeyDown }, ref) => {
    ...
    const activeDisplayValues = optionValues.filter((option) => (
        selectedValues.some((selected) => selected.toLowerCase() === option.toLowerCase())
    ));
    const displayLabel = activeDisplayValues.length === 0
        ? ALL_TYPES_VALUE
        : activeDisplayValues.length === 1
            ? activeDisplayValues[0]
            : `${activeDisplayValues.length} Types`;
    ...
    const toggleSelectedValue = (nextValue) => {
        const nextValues = selectedValues.some((selected) => selected.toLowerCase() === nextValue.toLowerCase())
            ? selectedValues.filter((selected) => selected.toLowerCase() !== nextValue.toLowerCase())
            : [...selectedValues, nextValue];

        onChange(buildTypeFilterValue(nextValues, optionValues));
    };
    ...
    const handleTriggerKeyDown = (event) => {
        if (!isOpen) {
            if (['Enter', ' ', 'ArrowDown', 'ArrowUp'].includes(event.key)) {
                ...
                if (selectedValues.length === 0) {
                    ...
                    const selectedIndex = optionValues.findIndex((option) => (
                        selectedValues.some((selected) => selected.toLowerCase() === option.toLowerCase())
                    ));
        ...
        switch (event.key) {
            case 'ArrowDown':
                event.preventDefault();
                setHighlightedIndex((prev) => (prev + 1) % optionValues.length);
                break;
            case 'ArrowUp':
                event.preventDefault();
                setHighlightedIndex((prev) => (prev - 1 + optionValues.length) % optionValues.length);
                break;
            case 'Enter':
                event.preventDefault();
                if (highlightedIndex >= 0 && highlightedIndex < optionValues.length) {
                    toggleSelectedValue(optionValues[highlightedIndex]);
                }
                break;
            case 'Escape':
                event.preventDefault();
    ...
    const triggerCompactClassName = 'px-2.5 gap-1.5 lg:px-2.5 lg:gap-1.5 2xl:px-2.5 2xl:gap-1.5';
    const labelClassName = displayLabel === ALL_TYPES_VALUE
        ? 'max-w-[116px] lg:max-w-[92px] 2xl:max-w-[116px]'
        : 'max-w-[92px] lg:max-w-[72px] 2xl:max-w-[92px]';
    const chevronCompactClassName = 'lg:ml-0.5 2xl:ml-0.5';
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`group relative flex items-center justify-between h-[30px] rounded-md border border-gray-200 bg-white text-slate-800 hover:bg-[#F8FAFF] hover:border-[#D7E6FF] focus:outline-none focus-visible:bg-[#F8FAFF] focus-visible:border-[#D7E6FF] focus-visible:ring-2 focus-visible:ring-blue-100 shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-all ${triggerCompactClassName}`}
            >
                <div className="flex items-center gap-1.5 min-w-0">
                    <Filter size={14} className="shrink-0 text-slate-400 group-hover:text-slate-500 group-focus-visible:text-slate-500 transition-colors" />
                    <span className={`${labelClassName} truncate text-[12px] font-medium`}>
                        {displayLabel}
                    </span>
                </div>
                <ChevronDown size={13} className={`shrink-0 transition-transform ${chevronCompactClassName} ${isOpen ? 'rotate-180 text-slate-500' : 'text-slate-400 group-hover:text-slate-500 group-focus-visible:text-slate-500'}`} />
            </button>

            {isOpen && dropdownPosition && createPortal(
                <div
                    ref={dropdownMenuRef}
                    className="fixed min-w-[184px] w-48 bg-white rounded-2xl shadow-[0_16px_44px_rgba(15,23,42,0.14)] border border-slate-100 py-2 z-[100] animate-in fade-in zoom-in-95 duration-200"
                    style={{ top: dropdownPosition.top, left: dropdownPosition.left }}
                >
                    <div className="max-h-[220px] overflow-y-auto custom-scrollbar px-1.5">
                        {optionValues.map((option, idx) => {
                            const isSelected = selectedValues.some((selected) => selected.toLowerCase() === option.toLowerCase());
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors ${isHighlighted ? 'bg-slate-50' : 'hover:bg-slate-50'}`}
                                >
                                    <div className="w-4 flex justify-center shrink-0">
                                        {isSelected && <Check size={16} className="text-[#4A8AF4]" strokeWidth={2.75} />}
                                    </div>
                                    <div className="min-w-0 truncate">
                                        <p className={`min-w-0 truncate tracking-tight text-[11px] ${isSelected ? 'font-bold text-slate-800' : 'font-medium text-slate-600'}`}>
                                            {option}
                                        </p>
                                    </div>
                                </button>
...
const ReportsCategorySelect = React.forwardRef(({ value, options, onChange, onKeyDown, multiple = false }, ref) => {
    const [isOpen, setIsOpen] = useState(false);
    ...
    const selectedValues = useMemo(() => normalizeCategoryFilterValues(value), [value]);
    const currentValue = Array.isArray(value)
        ? (value[0] || ALL_CATEGORIES_VALUE)
        : value && value !== ALL_CATEGORIES_VALUE
            ? value
            : ALL_CATEGORIES_VALUE;
    const isAllSelected = multiple
        ? isAllCategoryFilterSelected(value, optionValues)
        : currentValue === ALL_CATEGORIES_VALUE;
    ...
    const displayLabel = multiple
        ? (isAllSelected
            ...
            : displayValues.length === 1
                ? displayValues[0]
                : `${displayValues.length} Categories`)
        : currentValue;
    ...
    }, [currentValue, isAllSelected, isOpen, multiple, optionValues, selectedValues]);
    ...
    const handleApply = () => {
        onChange(
            multiple
                ? buildCategoryFilterValue(stagedValues, optionValues)
                : (stagedValue || ALL_CATEGORIES_VALUE)
        );
        setIsOpen(false);
        setHighlightedIndex(-1);
    };

    const toggleStagedValue = (categoryValue) => {
        setStagedValues((prev) => {
            const exists = prev.some((item) => item.toLowerCase() === categoryValue.toLowerCase());
            if (exists) {
                return prev.filter((item) => item.toLowerCase() !== categoryValue.toLowerCase());
            }
    ...
    const handleTriggerKeyDown = (event) => {
        if (!isOpen) {
            if (['Enter', ' ', 'ArrowDown', 'ArrowUp'].includes(event.key)) {
                event.preventDefault();
                setIsOpen(true);
                if ((multiple && isAllSelected) || (!multiple && currentValue === ALL_CATEGORIES_VALUE)) {
                    setHighlightedIndex(0);
                } else {
                    const selectedIndex = optionValues.findIndex((option) => (
                        multiple
    ...
    const labelClassName = displayLabel === ALL_CATEGORIES_VALUE
                ...
                >
                    <div className="px-3 py-1.5 border-b border-slate-100 flex items-center justify-between">
                        <button
                            ...
                            className={`group flex items-center gap-1.5 text-[11px] font-bold transition-colors uppercase tracking-wider rounded-md px-2 py-1 ${
                                multiple
                                    ? (stagedValues.length >= optionValues.length ? 'text-[#2F5FC6]' : 'text-slate-500 hover:text-slate-800')
                                    : (stagedValue === ALL_CATEGORIES_VALUE ? 'text-[#2F5FC6]' : 'text-slate-500 hover:text-slate-800')
                            }`}
                        >
                            <div className="w-4 flex justify-center shrink-0">
                                <Check
                                    ...
                                    className={`${
                                        multiple
                                            ? (stagedValues.length >= optionValues.length ? 'text-[#4A8AF4]' : 'text-slate-200 group-hover:text-slate-300')
                                            : (stagedValue === ALL_CATEGORIES_VALUE ? 'text-[#4A8AF4]' : 'text-slate-200 group-hover:text-slate-300')
                                    } transition-colors`}
                                    strokeWidth={
                                        multiple
                                            ? (stagedValues.length >= optionValues.length ? 3 : 2.5)
                                            : (stagedValue === ALL_CATEGORIES_VALUE ? 3 : 2.5)
                                    }
                                />
                            </div>
                            {ALL_CATEGORIES_VALUE}
                    ...
                    <div className="max-h-[128px] overflow-y-auto custom-scrollbar py-1">
                        {optionValues.map((option, idx) => {
                            const isSelected = multiple
                                ? stagedValues.some((item) => item.toLowerCase() === option.toLowerCase())
                                : stagedValue === option;
...
const ReportsAccountSelect = React.forwardRef(({ value, options, onChange, onKeyDown, multiple = false }, ref) => {
    const [isOpen, setIsOpen] = useState(false);
    ...
    const selectedValues = useMemo(() => normalizeAccountFilterValues(value), [value]);
    const currentValue = Array.isArray(value)
        ? (value[0] || ALL_ACCOUNTS_VALUE)
        : value && value !== ALL_ACCOUNTS_VALUE
            ? value
            : ALL_ACCOUNTS_VALUE;
    const isAllSelected = multiple
        ? isAllAccountFilterSelected(value, optionValues)
        : currentValue === ALL_ACCOUNTS_VALUE;
    const displayValues = multiple
        ? optionValues.filter((option) => selectedValues.some((selected) => selected.toLowerCase() === option.toLowerCase()))
        : [];
    const displayLabel = multiple
    ...
    const handleApply = () => {
        onChange(
            multiple
                ? buildAccountFilterValue(stagedValues, optionValues)
                : (stagedValue || ALL_ACCOUNTS_VALUE)
        );
        setIsOpen(false);
        setHighlightedIndex(-1);
    };

    const toggleStagedValue = (accountValue) => {
        setStagedValues((prev) => {
            const exists = prev.some((item) => item.toLowerCase() === accountValue.toLowerCase());
            if (exists) {
                return prev.filter((item) => item.toLowerCase() !== accountValue.toLowerCase());
            }
    ...
    const handleTriggerKeyDown = (event) => {
        if (!isOpen) {
            if (['Enter', ' ', 'ArrowDown', 'ArrowUp'].includes(event.key)) {
                event.preventDefault();
                setIsOpen(true);
                if ((multiple && isAllSelected) || (!multiple && currentValue === ALL_ACCOUNTS_VALUE)) {
                    setHighlightedIndex(0);
                } else {
                    const selectedIndex = optionValues.findIndex((option) => (
                        multiple
    ...
    const triggerCompactClassName = 'lg:px-2 lg:gap-1.5 2xl:px-3 2xl:gap-2';
    const labelClassName = displayLabel === ALL_ACCOUNTS_VALUE
        ? 'max-w-[150px] lg:max-w-[118px] 2xl:max-w-[150px]'
        : 'max-w-[120px] lg:max-w-[94px] 2xl:max-w-[120px]';
    const chevronCompactClassName = 'lg:ml-0.5 2xl:ml-1';
                    ...
                    <div className="px-3 py-1.5 border-b border-slate-100 flex items-center justify-between">
                        <button
                            ...
                            className={`group flex items-center gap-1.5 text-[11px] font-bold transition-colors uppercase tracking-wider rounded-md px-2 py-1 ${
                                multiple
                                    ? (stagedValues.length >= optionValues.length ? 'text-[#2F5FC6]' : 'text-slate-500 hover:text-slate-800')
                                    : (stagedValue === ALL_ACCOUNTS_VALUE ? 'text-[#2F5FC6]' : 'text-slate-500 hover:text-slate-800')
                            }`}
                        >
                            <div className="w-4 flex justify-center shrink-0">
                                <Check
                                    ...
                                    className={`${
                                        multiple
                                            ? (stagedValues.length >= optionValues.length ? 'text-[#4A8AF4]' : 'text-slate-200 group-hover:text-slate-300')
                                            : (stagedValue === ALL_ACCOUNTS_VALUE ? 'text-[#4A8AF4]' : 'text-slate-200 group-hover:text-slate-300')
                                    } transition-colors`}
                                    strokeWidth={
                                        multiple
                                            ? (stagedValues.length >= optionValues.length ? 3 : 2.5)
                                            : (stagedValue === ALL_ACCOUNTS_VALUE ? 3 : 2.5)
                                    }
                                />
                            </div>
                            {ALL_ACCOUNTS_VALUE}
...
const ReportsPartySelect = React.forwardRef(({ value, options, multiple = false, onChange, onKeyDown }, ref) => {
    const [isOpen, setIsOpen] = useState(false);
    ...
    const selectedValues = useMemo(() => normalizePartyFilterValues(value), [value]);
    const currentValue = Array.isArray(value)
        ? (value[0] || ALL_PARTIES_VALUE)
        : value && value !== ALL_PARTIES_VALUE
            ? value
            : ALL_PARTIES_VALUE;
    const isAllSelected = multiple
        ? isAllPartyFilterSelected(value, optionValues)
        : currentValue === ALL_PARTIES_VALUE;
    const displayValues = multiple
        ? optionValues.filter((option) => selectedValues.some((selected) => selected.toLowerCase() === option.toLowerCase()))
        : [];
    const displayLabel = multiple
    ...
    const handleApply = () => {
        onChange(
            multiple
                ? buildPartyFilterValue(stagedValues, optionValues)
                : (stagedValue || ALL_PARTIES_VALUE)
        );
        setIsOpen(false);
        setHighlightedIndex(-1);
    };

    const toggleStagedValue = (partyValue) => {
        setStagedValues((prev) => {
            const exists = prev.some((item) => item.toLowerCase() === partyValue.toLowerCase());
            if (exists) {
                return prev.filter((item) => item.toLowerCase() !== partyValue.toLowerCase());
            }
            return [...prev, partyValue];
        });
    };
    ...
    const handleTriggerKeyDown = (event) => {
        if (!isOpen) {
            if (['Enter', ' ', 'ArrowDown', 'ArrowUp'].includes(event.key)) {
                event.preventDefault();
                setIsOpen(true);
                if ((multiple && isAllSelected) || (!multiple && currentValue === ALL_PARTIES_VALUE)) {
                    setHighlightedIndex(0);
                } else {
                    const selectedIndex = optionValues.findIndex((option) => (
                        multiple
    ...
    const triggerCompactClassName = 'lg:px-2 lg:gap-1.5 2xl:px-3 2xl:gap-2';
    const labelClassName = displayLabel === ALL_PARTIES_VALUE
        ? 'max-w-[150px] lg:max-w-[118px] 2xl:max-w-[150px]'
        : 'max-w-[120px] lg:max-w-[94px] 2xl:max-w-[120px]';
    const chevronCompactClassName = 'lg:ml-0.5 2xl:ml-1';
                ...
                >
                    <div className="px-3 py-1 border-b border-slate-100 flex items-center justify-between">
                        <button
                            ...
                            className={`group flex items-center gap-1.5 text-[10px] font-bold transition-colors uppercase tracking-wider rounded-md px-2 py-0.5 ${
                                multiple
                                    ? (stagedValues.length >= optionValues.length ? 'text-[#2F5FC6]' : 'text-slate-500 hover:text-slate-800')
                                    : (stagedValue === ALL_PARTIES_VALUE ? 'text-[#2F5FC6]' : 'text-slate-500 hover:text-slate-800')
                            }`}
                        >
                            <div className="w-4 flex justify-center shrink-0">
                                <Check
                                    ...
                                    className={`${
                                        multiple
                                            ? (stagedValues.length >= optionValues.length ? 'text-[#4A8AF4]' : 'text-slate-200 group-hover:text-slate-300')
                                            : (stagedValue === ALL_PARTIES_VALUE ? 'text-[#4A8AF4]' : 'text-slate-200 group-hover:text-slate-300')
                                    } transition-colors`}
                                    strokeWidth={
                                        multiple
                                            ? (stagedValues.length >= optionValues.length ? 3 : 2.5)
                                            : (stagedValue === ALL_PARTIES_VALUE ? 3 : 2.5)
                                    }
                                />
                            </div>
                            {ALL_PARTIES_VALUE}
...
const Reports = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            categoryId = resolveCategoryFilterIds(filters.category, categories, {
                allowMultiple: doesReportTypeAllowMultipleCategories(filters.reportType)
            });

            accountId = resolveAccountFilterIds(filters.account, accounts, {
                allowMultiple: doesReportTypeAllowMultipleAccounts(filters.reportType)
            });
            party = resolvePartyFilterValue(filters.party, {
                allowMultiple: doesReportTypeAllowMultipleParties(filters.reportType)
            });
    ...
    const buildExportPayload = (format) => {
        ...
        categoryId = resolveCategoryFilterIds(exportFilters.category, categories, {
            allowMultiple: doesReportTypeAllowMultipleCategories(exportFilters.reportType)
        });
        accountId = resolveAccountFilterIds(exportFilters.account, accounts, {
            allowMultiple: doesReportTypeAllowMultipleAccounts(exportFilters.reportType)
        });
        party = resolvePartyFilterValue(exportFilters.party, {
            allowMultiple: doesReportTypeAllowMultipleParties(exportFilters.reportType)
        });
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
            ...
            <div className="w-[140px] shrink-0">
                <ReportsCategorySelect
                    ...
                    multiple={doesReportTypeAllowMultipleCategories(filters.reportType)}
            ...
            <div className="flex gap-2 relative no-print sm:ml-1 shrink-0">
                ...
                <button
                    ...
                    title="Export as PDF"
                >
                    <FileText size={14} className="text-gray-500 group-hover:text-[#4A8AF4] transition-colors" />
                    <span className="hidden sm:inline font-medium text-[12px]">PDF</span>
                </button>
            </div>
        </div>
    );
                                        ...
                                        {filters.reportType === 'Account-wise' && (
                                            <div className="w-full sm:w-auto hidden lg:block">
                                                <ReportsAccountSelect
                                                    ...
                                                    multiple={doesReportTypeAllowMultipleAccounts(filters.reportType)}
                                        ...
                                        {filters.reportType === 'Party-wise' && (
                                            <div className="w-full sm:w-auto hidden lg:block">
                                                <ReportsPartySelect
                                                    ...
                                                    multiple={doesReportTypeAllowMultipleParties(filters.reportType)}
            ...
            {!isPrinting && (
                <>
                    ...
                    <div className="reports-tablet-page flex-1 space-y-4 animate-in fade-in duration-500 print:hidden overflow-visible">
                        ...
                        <div className="no-print report-filters flex flex-col 2xl:flex-row 2xl:justify-between items-start 2xl:items-center gap-4 px-5 pt-3 w-full transition-all pb-4 relative z-20">
                            {!isProfitLossReport ? (
