...
const Transactions = () => {
    ...
    const colDefs = useMemo(() => {
        const baseDefs = [
            ...
            { field: 'party', headerName: 'Party', hide: !visibleColumns.party, minWidth: 150, cellRenderer: (params) => <PartyTooltip partyName={params.value} />, flex: 1.5, valueGetter: params => params.data?.contact || params.data?.payee || params.data?.counterpartyName || params.data?.party || params.data?.name || params.data?.referenceNumber || '-' },
    ...
    const fetchTransactions = React.useCallback(async (signal) => {
        ...
        try {
            ...
            if (appliedFilters.accountId && appliedFilters.accountId !== 'all') {
                payload.accountId = appliedFilters.accountId;
            }
            ...
            } else if (new URLSearchParams(typeof window !== 'undefined' ? window.location.search : '').get('allDates') === 'true') {
                payload.startDate = '1970-01-01';
                payload.endDate = '2099-12-31';
            }
    ...
    const executeExcelExport = () => {
        ...
        gridRef.current.api.forEachNodeAfterFilterAndSort(node => {
            ...
            TXN_TABLE_COLUMNS.forEach(col => {
                ...
                if (col.key === 'party') val = txn.contact || txn.payee || txn.counterpartyName || txn.party || txn.name || txn.referenceNumber || '-';
        ...
        if (printContainer && gridRef.current && gridRef.current.api) {
            ...
            gridRef.current.api.forEachNodeAfterFilterAndSort(node => {
                ...
                TXN_TABLE_COLUMNS.forEach(col => {
                    ...
                    let val = '-';
                    if (col.key === 'date') val = formatDate(txn.txnDate);
                    if (col.key === 'party') val = `<span class="truncate block max-w-[200px]">${txn.contact || txn.payee || txn.counterpartyName || txn.party || txn.name || txn.referenceNumber || '-'}</span>`;
                    if (col.key === 'type') val = txn.transactionType?.name || txn.txnType || '-';
                    if (col.key === 'branch') val = txn.branchNames?.length > 0 ? txn.branchNames.join(', ') : '-';
                    if (col.key === 'account') val = txn.account?.name || '-';
                    if (col.key === 'category') val = txn.category?.name || '-';
