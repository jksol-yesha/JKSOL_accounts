...
export const buildAttachmentUrl = (path) => {
    ...
    if (!rawPath) return '';
    if (/^https?:\/\//i.test(rawPath)) return encodeURI(rawPath);

    const normalizedPath = rawPath
        .replace(/\\/g, '/')
        .replace(/^https?:\/\/[^/]+/i, '');

    const apiUploadsIndex = normalizedPath.lastIndexOf('/api/uploads/');
    if (apiUploadsIndex !== -1) {
        return encodeURI(`${API_ORIGIN}${normalizedPath.slice(apiUploadsIndex)}`);
    }

    const uploadsIndex = normalizedPath.lastIndexOf('/uploads/');
    if (uploadsIndex !== -1) {
        return encodeURI(`${API_ORIGIN}/api${normalizedPath.slice(uploadsIndex)}`);
    }

    const trimmedPath = normalizedPath.replace(/^\/+/, '');
    if (trimmedPath.startsWith('api/uploads/')) {
        return encodeURI(`${API_ORIGIN}/${trimmedPath}`);
    }

    if (trimmedPath.startsWith('uploads/')) {
        return encodeURI(`${API_ORIGIN}/api/${trimmedPath}`);
    }

    return encodeURI(`${API_BASE_URL}${normalizedPath.startsWith('/') ? '' : '/'}${normalizedPath}`);
};
...
export const getAttachmentFileName = (path) => {
    ...
    const lastSegment = rawPath.split('/').pop() || 'attachment';
    const decodedName = decodeURIComponent(lastSegment);
    const cleanedName = decodedName.split('-').slice(5).join('-');

    return cleanedName || decodedName || 'attachment';
};

export const downloadAttachmentFile = async (path) => {
    if (!path) {
        throw new Error('Attachment path is required');
    }

    const downloadUrl = buildAttachmentUrl(path);
    ...
    downloadLink.download = getAttachmentFileName(path);
    ...
    async (config) => {
        ...
        if (storedOrg) {
            try {
                ...
                // Ignore malformed persisted org payloads.
            }
        }
        ...
        if (!shouldSkipBranch) {
            try {
                ...
                // Ignore malformed persisted branch payloads.
            }
        }
        ...
        if (storedPrefs) {
            try {
                ...
                // Ignore malformed persisted preference payloads.
            }
        }
...
api.interceptors.response.use(
    async (response) => {
        if (response.headers?.['x-crypto-key-refresh'] === '1') {
            clearPublicKey();
        }
